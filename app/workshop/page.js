...
import { Camera, Search, User, X, RotateCcw, CheckCircle, BarChart3, LayoutDashboard } from 'lucide-react'
...
export default function WorkshopDashboard() {
  ...
  // 1. Fetch Global Logs for Analytics
  useEffect(() => {
    if (view === 'stats') {
      fetchGlobalLogs()
    }
            ...
            <div className={`bg-white border-4 p-6 rounded-3xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] ${activeOrder.is_rush ? 'border-red-600 ring-4 ring-red-200' : 'border-black'}`}>
              ...
              <div className="space-y-3 mb-8">
                ...
                {activeOrder.current_stage === 'QC' && !showRejectMenu && (
                  <button onClick={() => setShowRejectMenu(true)} className="w-full bg-orange-100 text-orange-600 p-4 border-2 border-orange-600 rounded-2xl font-black flex items-center justify-center gap-2"><RotateCcw size={20} /> REJECT / NEEDS FIX</button>
                )}
                  ...
                  <div className="bg-orange-50 p-4 rounded-2xl border-2 border-orange-200">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {REDO_REASONS.map(reason => (
                        <button key={reason} onClick={() => handleMove('Goldsmithing', true, reason)} className="bg-white p-3 border-2 border-black rounded-xl font-bold text-sm hover:bg-orange-500 hover:text-white text-left">{reason}</button>
                      ))}
                      <button onClick={() => setShowRejectMenu